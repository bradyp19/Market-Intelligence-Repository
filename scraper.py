...
class ConfidenceScorer:
    ...
    def _score_announcement_strength(self, text: str, title: str, company: str) -> float:
...
class AnnouncementScraper:
                ...
                if content:
                    ...
                    if manual_result:
                        result = {
                            ...
                            'date': None  # Manual extraction doesn't get publish date
                        }
    ...
    def scrape_company(self, company: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Process both blog and press URLs
            for url_type in ['blog_url', 'press_url']:
                if url_type not in company_config:
                    continue
                    
                announcements.extend(
                    self._process_url_source(company_config[url_type], company, processed_urls, all_articles)
                )
                
                if len(announcements) >= 5:
                    break
            
            # Fallback: use most recent article if no announcements found
    ...
    def _process_url_source(self, base_url: str, company: str, processed_urls: set, all_articles: list) -> List[Dict[str, Any]]:
        ...
        for link in links:
            if link in processed_urls or len(announcements) >= 5:
                break
